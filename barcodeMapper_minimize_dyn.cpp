...
for(itrbw=best_windows.begin();itrbw!=best_windows.end(); itrbw++){
  std::cout<<"\nquality: " << (to_string(std::get<0>(*itrbw))+(" "*(to_string(std::get<0>(*itrbw)).length()-20))) << "\tref: " << std::get<1>(*itrbw)<< "\tstart: "<< std::get<2>(*itrbw) << "\tend: " << std::get<3>(*itrbw) << "\tlength: " << std::get<3>(*itrbw)-std::get<2>(*itrbw);
}
std::cerr<<"\n";
