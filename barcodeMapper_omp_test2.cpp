...
*/
void MapKmerList(std::vector<std::tuple<uint_fast8_t,uint32_t,uint32_t,uint32_t>> & kmer_list, uint_fast32_t & max_window_size, uint_fast32_t & max_gap_size, uint_fast8_t & window_count, const char* file, DnaString barcode, unsigned qualityThreshold, unsigned lengthThreshold, std::vector<std::tuple<double,uint_fast8_t,uint32_t,uint32_t>> & best_windows);
...
struct bcmapOptions{
  ...
  std::string protocol;
  bcmapOptions() :
  k(31), mini_window_size(35), output_file("barcode_windows.bed"),l(1000) , q(20000), protocol("10x")
  {}
  };
...
seqan::ArgumentParser::ParseResult parseCommandLine(bcmapOptions & options, int argc, char const ** argv){
    ...
    getOptionValue(options.protocol, parser, "p");
...
BCI_posSet.resize(3,{});
std::vector<std::vector<std::tuple<double,uint_fast8_t,uint32_t,uint32_t>>> best_windows;
best_windows.resize(3,{}); //(maping_quality, reference, start position in referende, end position)
...
while (!atEnd(file1)){ // reading and processing next batch of reads until file endpos
  ...
  #pragma omp parallel //num_threads(16)
  {
    ...
    for (int i=-3; i<(int)barcodeSet[thread3].size(); i++) {// for all barcodes in set

      if (i==-3){
          std::vector<std::tuple<double,uint_fast8_t,uint32_t,uint32_t>>::iterator itrbw;
          std::fstream results;
          results.open(toCString(options.output_file),std::ios::out | std::ios::app);

          for(itrbw=best_windows[thread4].begin();itrbw!=best_windows[thread4].end(); itrbw++){
          ...
          results.close();
          thread4=(thread4+1)%3; // iterate thread
      }
...
for(int i=-1;i<2;i++){
  ...
  if (i==1){   // process reads and write results to file
    ...
    for (itrreadSetG = readSet[thread3].begin(), itrbarcG=barcodeSet[thread3].begin(); itrreadSetG != readSet[thread3].end(); itrreadSetG++ ,itrbarcG++) {// for all barcodes in set
      std::vector<std::tuple<uint_fast8_t,uint32_t,uint32_t,uint32_t>> kmer_list;   // (i,j,a,m_a)   i=reference (Chromosome), j=position of matching k-mer in reference, a=abundance of k-mer in reference, m_a=minimizer_active_bases
      for (std::vector<Dna5String>::iterator it = (*itrreadSetG).begin(); it!=(*itrreadSetG).end(); ++it){                                            // Iterating over the reads
        std::pair <int64_t, int64_t> hash = hashkMer(infix(*it,0,k),k);                                // calculation of the hash value for the first k-mer
        int64_t minimizer_position=0;
        int64_t minimizer = InitMini(infix(*it,0,mini_window_size), k, hash, maxhash, random_seed, minimizer_position);          // calculating the minimizer of the first window
        uint_fast8_t minimizer_active_bases=1;
